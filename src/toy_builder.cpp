...
namespace __impl {
  template<auto... vals>
  struct replicator_type_expand_all {
    template<typename F>
  ...
  template<auto... vals>
  replicator_type<vals...> replicator_expand = {};
}
...
// Concept that checks if a type is a container but not a string (because string handling is required to be different)
template<typename T>
concept ContainerButNotString = requires(T a) {
    { a.size() } -> std::convertible_to<std::size_t>;
    { a[std::declval<std::size_t>()] }; // check if elements are accessible for the subscript operator
} && !std::is_same_v<T, std::string> && !std::is_same_v<T, std::string_view> && !std::is_same_v<T, const char*>;
