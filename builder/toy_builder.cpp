/* Notes
    * This is a toy builder that takes a struct and converts it to a json string (supports basic types thanks to Daniel Lemire!)
    * It uses the new reflection TS and the new ranges TS
    * It depends on an EDG-Experimental version of the compiler that partially implements the Reflection for C++ 26 proposal (https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2023/p2996r1.html#converting-a-struct-to-a-tuple)
    * You can run the code on the compiler explorer (https://godbolt.org/z/EKPq7jGK9)
*/
...
// The following 3 functions were taken from: https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2023/p2996r1.html#converting-a-struct-to-a-tuple
namespace __impl {
  template<auto... vals>
...
std::string to_json_string(array& desc, const T& t,  std::index_sequence<i...>)
{
    std::string s = "{";
    std::string quote = "\"";
    (..., (s += (i == 0? "" : ", ") + quote + std::string(desc[i].field_name)  + quote +":"+ atom(std::get<i>(t)) ));
    s += "}";
    return s;
}
...
{
    std::cout << "{";
    (..., (std::cout << (i == 0? "" : ", ") << '"'  << escape_json(desc[i].field_name) <<'"' << ':' << std::get<i>(t)));
    std::cout << "}" << std::endl;
}
